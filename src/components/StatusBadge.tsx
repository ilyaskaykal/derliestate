...
export const musteriConfig: Record<MusteriDurum, { label: string; bg: string; color: string; shadow: string; dotColor: string }> = {
...
export function MusteriStatusBadge({ durum }: { durum: MusteriDurum }) {
  ...
  return (
    <span style={{ background: c.bg, color: c.color, padding: '2px 8px', borderRadius: 99, fontSize: 11, fontWeight: 600, whiteSpace: 'nowrap' }}>
      {c.label}
    </span>
  );
}
